...
void CMarkup::show()
{
	::MessageBox(NULL,"a","end",MB_OK);
	for(int i=0;i<=m_nElements;i++)
	{
		char strE[80];		
		sprintf(strE,"m_pElements[%d].iNext= %d\n",i,m_pElements[i].iNext);
		::MessageBox(NULL,strE,"ALL m_pElements",MB_OK);
	}
}
...
bool CMarkup::_Parse(LPTSTR& pstrText, ULONG iParent)
{
    ...
    for( ; ; ) 
    {
        ...
        *pstrText++ = _T('\0');
		...
		sprintf(strpos,"iPrevious:%d -- iPos: %d\niNext: %d",iPrevious,iPos,pEl->iNext);
		//::MessageBox(NULL,strpos,"iPrevious",MB_OK);
        if( iPrevious != 0 )
		{
			char strEdit[80];
			sprintf(strEdit,"iPrevious:%d\n",iPrevious);
			//::MessageBox(NULL,strEdit,"iPrevious",MB_OK); 
			m_pElements[iPrevious].iNext = iPos;
			sprintf(strEdit,"m_pElements[%d].iNext= %d\n",iPrevious,m_pElements[iPrevious].iNext);
			//::MessageBox(NULL,strEdit,"iPrevious",MB_OK);
		}
		...
		//::MessageBox(NULL,pstrName,"iPrevious",MB_OK); 
...
CMarkup::XMLELEMENT* CMarkup::_ReserveElement()
{
    if( m_nElements == 0 )
		{
			//char aa[80];
			//sprintf(aa,"iPrevious:%d\n",iPrevious);
			//::MessageBox(NULL,"here","cmarkupreif1",MB_OK);
    ...
    if( m_nElements >= m_nReservedElements ) {
		...
		//char strEdit[80];
		//sprintf(strEdit,"iPrevious:%d\n",iPrevious);
		//::MessageBox(NULL,"there","cmarkupreif2",MB_OK);
	}

	char st[80];
	sprintf(st,"This is m_nElements:%d\n",m_nElements);
	//::MessageBox(NULL,st,"m_nElements",MB_OK);
